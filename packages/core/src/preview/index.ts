import type { DownloadMode, FormatPreset, ProviderId } from "@pinforge/types";
import { detectProvider, ProviderNotFoundError } from "@pinforge/providers";
import {
  isBoardUrl,
...
import { DEFAULT_YOUTUBE_OPTIONS } from "@pinforge/types";
...
export async function extractMediaPreview(
  ...
  if (provider.id === "youtube" && mode === "single") {
    try {
      const { previewYouTubeVideo } = await import("@pinforge/providers/youtube/service");
  ...
  if (provider.id === "ytdlp" && mode === "single") {
    try {
      const { previewYtdlp } = await import("@pinforge/providers/ytdlp");
