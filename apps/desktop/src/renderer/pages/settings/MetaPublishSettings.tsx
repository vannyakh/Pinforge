...
const MetaPublishSettings: React.FC = () => {
  ...
  if (loading || !publicConfig) {
    return <SettingsLoading label="Loading Facebook settings…" />;
  }
    ...
    <>
      ...
      <SettingsSection title="Media uploads">
        <SettingsRow
          ...
          description="Photo uploads can use your Cloudflare tunnel (Settings → Remote). Videos are uploaded directly from this device — Meta often cannot fetch video from tunnel URLs."
        >
          <Button type="outline" onClick={() => navigate("/settings/remote")}>
            Remote settings
          </Button>
        </SettingsRow>
      </SettingsSection>
      ...
      <SettingsSection title="Test post">
        ...
        <SettingsField title="Media file" description="Optional image or video. Video uploads go directly to Meta; photos may use Cloudflare Tunnel when enabled.">
